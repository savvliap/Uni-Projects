...
public class SavvasLiapis extends FPlayer {
	...
	static String firstname = "ÓÜââáò";
	static String lastname = "ËéÜðçò";
	...
	public int nextMove(FacilityGameAPI game) 
	{
		...
		if (game.getMoveLocation().size()==1)lastBmove=0; 
		else lastBmove=game.getMoveLocation().get(game.getCurMoveIndex()-2);
		//to xreiazomai giati apo auto dialegw an tha kanw kinhsh deksia h aristera 
		int lastAmove=game.getMoveLocation().get(game.getCurMoveIndex()-1);
		...
		if (lastBmove>=n-4)
		{
			for (int i=lastBmove-3;i<n-1;i++) game.getStatusCopy()[i]=EnumFacilityStatus.BLOCKED;
		}
		//an einai sthn arxi tou pinaka vazw ton periorismo gia na mhn mou vgazei pali sfalma 
		else if (lastBmove<=3)
		{
			for (int i=0;i<lastBmove+3;i++) game.getStatusCopy()[i]=EnumFacilityStatus.BLOCKED;
		}
		else for(int i=lastBmove-3;i<lastBmove+3;i++)game.getStatusCopy()[i]=EnumFacilityStatus.BLOCKED;
	
		//h prwti kinisi mou 
		if (game.getStatusCopy()[middle]==EnumFacilityStatus.FREE )move=middle;
		...
		else if (lastAmove<middle)
		{
			...
			for (int i=0;i<middle;i++) 
			{
				...
				if (game.getStatusCopy()[i]==EnumFacilityStatus.PLAYER_A && game.getStatusCopy()[i+2]==EnumFacilityStatus.PLAYER_A && game.getStatusCopy()[i+4]==EnumFacilityStatus.FREE)
				{
					move=i+4;
					break; 
				}
			...
			//to vazw na ksekinaei apo tin mesi kai na apomakrunetai gt me noiazei na kalupsw theseis konta sto kentro  
		     ...
		     {
		    	 //an exei parei timi h move den thelw na to treksei 
		    	 if (move!=0) break;
		    	 //an den exei parei timi h move thelw na to treksei 
		    	 else
		    	 {	
		    		 if(game.getStatusCopy()[j]== EnumFacilityStatus.FREE )
						{
							move=j;
							break;
						}
		...
		else if (lastAmove>middle)
		{
				...
				//thelw na treksei apo thn mesi  gt me endiaferoun oi mesaies theseis gia combos 
			     ...
			     {
			    	 //an exw valei timi sthn move den thelw na treksei 
			    	 if (move!=0) break;
			    	 //alliws thelw na treksei 
			    	 else
			    	 {	
			    		 if (lastBmove==middle) 
			    		 {
			    		 ...
			    		 else if(game.getStatusCopy()[j]== EnumFacilityStatus.FREE )
			    		 {
			    			 move=j;
			    			 break;
			    		 }
...
// ti na alaksw :na dimiourghsw metavlhtes gia ta makrinaria pou xrhsimopoiw 
//na svisw autes tis etoimes entoles pou mou exei den me vohthane pouthena 
//isws na ftiaksw sunarthsh gia to switching alla oxi toso anagkaio 
