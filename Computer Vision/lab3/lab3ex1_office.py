...
for r in range(570,630):
    first_batch=cv2.imread("lab3/office/input/in%06d.jpg" % r)
    fbg=cv2.cvtColor(first_batch, cv2.COLOR_BGR2GRAY)
    BUF[:,:,k] = fbg
    k+=1

#now getting the images 
for i in range(570,2050):
    k=i+1
    I = cv2.imread("lab3/office/input/in%06d.jpg" % i)
    ...
    #masks loading
    GTB=cv2.imread("lab3/office/groundtruth/gt%06d.png" % i)
