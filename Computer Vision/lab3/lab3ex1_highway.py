...
for r in range(470,530):
    first_batch=cv2.imread("lab3/highway/input/in%06d.jpg" % r)
    fbg=cv2.cvtColor(first_batch, cv2.COLOR_BGR2GRAY)
    fbg=cv2.resize(fbg,(360,240))
...
for i in range(470,1700):
    ...
    (T, thresh2) = cv2.threshold(MEDIAN_DIFF,10,255,cv2.THRESH_BINARY)

    #thresh2=cv2.resize(thresh2,(360,240))
