...
tenOne = torch.FloatTensor(numpy.ascontiguousarray(numpy.array(PIL.Image.open('input/in%06d.jpg' % 1))[:, :, ::-1].transpose(2, 0, 1).astype(numpy.float32) * (1.0 / 255.0)))
iStep = 1

if __name__ == '__main__':
	for i in range(2,1100,iStep):
		tenTwo = torch.FloatTensor(numpy.ascontiguousarray(numpy.array(PIL.Image.open('input/in%06d.jpg' % i))[:, :, ::-1].transpose(2, 0, 1).astype(numpy.float32) * (1.0 / 255.0)))
