...
rbfmodel1=fitcsvm(train1(:,1:3), train1(:,4), 'KernelFunction', 'rbf');
svm_3d_plot(rbfmodel1,data1(:,1:3),data1(:,4)); title('setosa vs all'); hold on;
...
rbfmodel2=fitcsvm(train2(:,1:3), train2(:,4), 'KernelFunction', 'rbf');
svm_3d_plot(rbfmodel2,data2(:,1:3),data2(:,4)); title('vercicolor vs all'); hold on;
...
rbfmodel3=fitcsvm(train3(:,1:3), train3(:,4), 'KernelFunction', 'rbf');
svm_3d_plot(rbfmodel3,data3(:,1:3),data3(:,4));title('virginica vs all'); hold on;

%% errors 

rbftrain_err= multidim_err(rbfmodel1, rbfmodel2, rbfmodel3, xtrain, ytrain)
rbftest_err= multidim_err(rbfmodel1, rbfmodel2, rbfmodel3, xtest, ytest)
rbfvalid_err= multidim_err(rbfmodel1, rbfmodel2, rbfmodel3, xvalidat, yvalidat)
